...
ULineNotifySettings::ULineNotifySettings(const FObjectInitializer& ObjectInitializer)
	...
	, bUseCompilationNotification(true)
	, bInsertProjectNameInCompilationNotification(true)
	, bSendDumpedLogs(true)
	...
	, bUseLightBuildNotification(true)
	, bInsertProjectNameInLightBuildNotification(true)
	, LightBuildSucceeded(TEXT("Light build succeeded"))
	, LightBuildFailed(TEXT("Light build failed"))
	, bAddStampToNotification(false)
